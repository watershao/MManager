...
import com.qrobot.mm.util.ByteUtil;
...
public class ImageManager {
	...
	private ErrorDataListener errorDataListener = new ErrorDataListener() {
		...
		public void OnErrorDataListener(int errorCode, byte[] msg) {
			...
			if (errorCode == 2) {
				Toast.makeText(mContext, R.string.prompt_Q_offline, Toast.LENGTH_LONG).show();
			}
		}
	};
...
//						fileName = fileName.substring(fileName.lastIndexOf("/"), fileName.length());
					...
					if (count!= 0 && count == currTime) {
						mHandler.obtainMessage(PhotoConstant.WHAT_FILENAME_IMAGES, fileName).sendToTarget();
						fileName = null;
						count = 0;
						currTime = 0;
						if (fos != null) {
	 ...
	 * 获取所有图片文件列表
	 */
	public int getCameraImages(){
		String imgFlag = "img:rl#camera";
		String cmd = imgFlag;
		
		int bSend = sendCmd(FLAG_TEXT, cmd);
	 ...
	 * 获取所有图片文件列表
	 */
	public int getCameraImages(boolean del){
		String imgFlag = "img:rl#camera";
		String cmd = imgFlag;
...
　　  	 * 获取原图数据(single picture)
	 * @param fileName
	 */
	public int getSingleImage(String fileName){
		String imgFlag = "img:sp#camera*";
		String sendData = imgFlag+fileName;
		
		int bSend = sendCmd(FLAG_TEXT, sendData);
		return bSend;
	}
...
　　   	 * 删除图片(*后面不带图片名称,表示删除所有图片)
	 ...
	 * @param fileNameList
	 */
	private int deleteImages(List<String> fileNameList){
	 ...
	 */
	public int getThumbImage(String fileName){
	 ...
	 * @param cmd
	 */
	private int sendCmd(String flag,String cmd){
		byte[] cmdByte = getSendByte(flag, cmd);
		if (cmdByte == null) {
			return -2;
		}
		return nClientManager.sendUserData(nClientManager.getRemoteId(), cmdByte, cmdByte.length);
	}
...
//			Log.d(TAG, files[1]+"len:"+files.length+"fl2:"+files[0]);
				...
				if (newList != null && newList.size() > 0) {
					...
					mHandler.obtainMessage(PhotoConstant.WHAT_NEW_IMAGES, newList).sendToTarget();
				} else {
					mHandler.obtainMessage(PhotoConstant.WHAT_NEW_IMAGES, new ArrayList<String>()).sendToTarget();
				}
				...
				String fl = msg.substring(7);
				Log.d(TAG, "fl:"+fl);
				mHandler.obtainMessage(PhotoConstant.WHAT_DELETE_IMAGES,fl).sendToTarget();
			}
		}
	}
